...
nextButton.addEventListener('click', function () {
    const nextImage = document.querySelector('#web-tech-image')
    if (prevIndex < WEB_TECH_IMAGES.length - 1) {
        prevIndex++;
        nextImage.setAttribute('src', WEB_TECH_IMAGES[prevIndex]);
    } else {
        prevIndex = 0;
        nextImage.setAttribute('src', WEB_TECH_IMAGES[prevIndex]);
    }
...
prevButton.addEventListener('click', function () {
    const prevImage = document.querySelector('#web-tech-image')
    prevIndex--;
    if (prevIndex < 0) {
        prevImage.setAttribute('src', WEB_TECH_IMAGES[WEB_TECH_IMAGES.length - 1]);
        prevIndex = WEB_TECH_IMAGES.length - 1;
    } else {
        prevImage.setAttribute('src', WEB_TECH_IMAGES[prevIndex]);
    }
});
